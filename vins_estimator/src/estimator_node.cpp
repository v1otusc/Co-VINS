...
/**
 * @brief: 从 IMU 测量值 imu_msg 和上一个 PVQ 递推得到当前 PVQ
 * @param IMU
 * @return {*}
 */
      ...
      if (SWARM_AGENT) {
        if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR &&
          ...
          if ((tmp_agent_t - last_agent_t).norm() > 0.05) {
            ...
            // ROS_WARN("preprocess agent frame time %f",
            // pubAgentFrame_time.toc());
...
void agent_process() {
  while (1) {
    ...
    if (pub_flag) {
      ...
      m_image_buf.lock();
      while (!image_buf.empty() && image_buf.front()->header.stamp.toSec() <
                                       tmp_msg.header.stamp.toSec())
        image_buf.pop();
      if (!image_buf.empty()) {
      ...
      m_image_buf.unlock();
      if (image_msg == NULL ||
          image_msg->header.stamp.toSec() != tmp_msg.header.stamp.toSec()) {
...
int main(int argc, char **argv) {
  // ros 初始化以及设置句柄，节点名称为 vins_estimator
  ...
  //? 这句话看来是有点多余了，readParameters(n) 中已经带有读取 param 为
  // config_file 的功能了
