...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg) {
  ...
  // frequency control
  // 发布频率控制，保证每秒钟处理的 img_msg 小于 FREQ，默认为频率控制在 10Hz
  // 以内 并不是每读入一帧图像，就要发布此帧的特征点 / 图像
  // 判断间隔内的发布次数
  if (round(1.0 * pub_count /
            (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ) {
    PUB_THIS_FRAME = true;
    // reset the frequency control
    // 时间间隔内的发布次数太多十分接近设定频率（大于 9.9 /
    // FREQ）时，更新时间间隔起始时刻，并将数据发布次数置 0
  ...
  // 将特征点id，矫正后归一化平面的3D点 (x,y,z=1)，像素2D点 (u,v)，像素的速度
  // (vx,vy)，封装成 sensor_msgs::PointCloudPtr 类型的 feature_points 实例，通过
  // pub_img 发布
  if (PUB_THIS_FRAME) {
    pub_count++;
      ...
      for (int i = 0; i < NUM_OF_CAM; i++) {
        ...
        for (unsigned int j = 0; j < trackerData[i].cur_pts.size(); j++) {
          ...
          /*
             ...
             trackerData[i].pts_velocity[j].y); Vector3d tmp_prev_un_pts;
                    tmp_prev_un_pts.head(2) = tmp_cur_un_pts - 0.10 *
             tmp_pts_velocity; tmp_prev_un_pts.z() = 1; Vector2d tmp_prev_uv;
                    trackerData[i].m_camera->spaceToPlane(tmp_prev_un_pts,
             tmp_prev_uv); cv::line(tmp_img, trackerData[i].cur_pts[j],
             cv::Point2f(tmp_prev_uv.x(), tmp_prev_uv.y()), cv::Scalar(255 , 0,
             0), 1 , 8, 0);
                    */
          // char name[10];
...
int main(int argc, char **argv) {
  ...
  // 读取 yaml 文件中的参数
  // 其中参数 config_file 在 launch 文件被设置为 $(find
  // feature_tracker)/../config/mynteye/mynteye_config.yaml
  readParameters(n);
  ...
  if (FISHEYE) {
    for (int i = 0; i < NUM_OF_CAM; i++) {
      ...
      // cv::imread(, 0) 读入图像单通道，即灰度图
      trackerData[i].fisheye_mask = cv::imread(FISHEYE_MASK, 0);
      // TODO: 确认 data: Mat对象中的一个指针类型的成员变量，指向存放 Mat
      // 矩阵数据的首地址(uchar* data)，是一个地址的指针
  ...
  pub_img =
      n.advertise<sensor_msgs::PointCloud>("feature_tracker/feature", 1000);
  // 跟踪的特征点图像，给 rviz 和调试用
  // pub_match =
  // n.advertise<sensor_msgs::Image>("feature_tracker/feature_img",1000);
