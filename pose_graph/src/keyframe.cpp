...
Matrix3d &_vio_R_w_i, Vector3d &_T_w_i, Matrix3d &_R_w_i, cv::Mat &_image, int
_loop_index, Eigen::Matrix<double, 8, 1 > &_loop_info, vector<cv::KeyPoint>
&_keypoints, vector<cv::Point2f> &_feature_2d, vector<BRIEF::bitset>
&_feature_des)
{
...
KeyFrame::KeyFrame(int _seq, double _time_stamp, Vector3d &_vio_T_w_i,
                   Matrix3d &_vio_R_w_i, Vector3d &_tic, Matrix3d &_ric,
                   vector<cv::Point3f> &_point_3d,
                   vector<cv::Point2f> &_feature_2d,
                   vector<BRIEF::bitset> &_point_des,
                   vector<BRIEF::bitset> &_feature_des) {
  sequence = _seq;
  time_stamp = _time_stamp;
  vio_T_w_i = _vio_T_w_i;
  vio_R_w_i = _vio_R_w_i;
...
void KeyFrame::searchByBRIEFDes(
    std::vector<cv::Point2f> &matched_2d_old, std::vector<uchar> &status,
    const std::vector<BRIEF::bitset> &descriptors_old,
    const std::vector<cv::Point2f> &feature_2d_old) {
...
void KeyFrame::PnPRANSAC(const vector<cv::Point2f> &matched_2d_old,
  ...
  if (CV_MAJOR_VERSION < 3)
    solvePnPRansac(matched_3d, matched_2d_old, K, D, rvec, t, true, 100,
  ...
  for (int i = 0; i < (int)matched_2d_old.size(); i++) status.push_back(0);
...
bool KeyFrame::findConnection(KeyFrame *old_kf) {
  ...
  // TicToc t_des;
  searchByBRIEFDes(matched_2d_old, status, old_kf->feature_des,
                   old_kf->feature_2d);
...
void KeyFrame::setLoop(int _loop_index,
