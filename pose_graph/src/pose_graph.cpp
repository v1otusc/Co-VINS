...
void PoseGraph::addAgentFrame(KeyFrame *cur_kf) {
  ...
  vio_P_cur =
      sequence_w_r_s_map[sequence] * vio_P_cur + sequence_w_t_s_map[sequence];
  ...
  if (loop_index != -1) {
    ...
    if (find_connection) {
      ...
      if (old_kf->sequence != cur_kf->sequence &&
        ...
        ROS_WARN("align %d sequence and %d sequence to world frame", sequence,
                 old_kf->sequence);
        sequence_align_world[sequence] = 1;
        ...
        vio_P_cur = sequence_w_r_s_map[sequence] * vio_P_cur +
                    sequence_w_t_s_map[sequence];
        vio_R_cur = sequence_w_r_s_map[sequence] * vio_R_cur;
        cur_kf->updateVioPose(vio_P_cur, vio_R_cur);
      ...
      if (old_kf->sequence != cur_kf->sequence &&
          sequence_align_world[old_kf->sequence] == 0 &&
          sequence_align_world[sequence] == 1) {
        ROS_WARN("align %d sequence and %d sequence to world frame",
        ...
        for (size_t i = 0; i < keyframe_vec.size(); i++) {
          ...
          if (it->sequence == old_kf->sequence) {
            ...
            vio_P = sequence_w_r_s_map[old_kf->sequence] * vio_P +
                    sequence_w_t_s_map[old_kf->sequence];
  ...
  if (SAVE_LOOP_PATH) {
    ...
    std::string pose_graph_sequence_path =
        VINS_RESULT_PATH + "/pose_graph_path_" + to_string(sequence) + ".csv";
    ...
    sequence_path_file << P.x() << "," << P.y() << "," << P.z() << "," << Q.w()
                       << "," << Q.x() << "," << Q.y() << "," << Q.z() << ","
                       << endl;
    ...
    if (cur_index != -1) {
      ...
      for (size_t k = first_looped_index; k < keyframe_vec.size(); k++) {
        ...
        for (int n = 0; n < 5; n++) {
          while (prev_index >= first_looped_index) {
            if (keyframe_vec[prev_index]->sequence == it->sequence) {
              ...
              Vector3d relative_t(t_array[i][0] - t_array[local_prev_index][0],
                                  t_array[i][1] - t_array[local_prev_index][1],
                                  t_array[i][2] - t_array[local_prev_index][2]);
              Quaterniond q_i_j = Quaterniond(
                  q_array[local_prev_index][0], q_array[local_prev_index][1],
                  q_array[local_prev_index][2], q_array[local_prev_index][3]);
              ...
              problem.AddResidualBlock(
                  vo_function, NULL, q_array[local_prev_index],
                  t_array[local_prev_index], q_array[i], t_array[i]);
        ...
        if ((it)->has_loop && sequence_align_world[(it)->sequence] == 1 &&
            sequence_align_world[getKeyFrame((it)->loop_index)->sequence] ==
                1)  // different 4dof
        {
      ...
      for (iter = last_sequence_index.begin();
           iter != last_sequence_index.end(); iter++) {
        ...
        sequence_t_drift_map[sequence] =
            cur_t - sequence_r_drift_map[sequence] * vio_t;
        m_drift.unlock();
      }
...
void PoseGraph::updatePath() {
  ...
  for (iter = sequence_align_world.begin(); iter != sequence_align_world.end();
       iter++)
  ...
  if (SAVE_LOOP_PATH) {
    ...
    map<int, bool>::iterator iter;
    for (iter = sequence_align_world.begin();
         iter != sequence_align_world.end(); iter++) {
      std::string pose_graph_sequence_path = VINS_RESULT_PATH +
                                             "/pose_graph_path_" +
                                             to_string(iter->first) + ".csv";
  ...
  for (size_t k = 0; k < keyframe_vec.size(); k++) {
    ...
    if (SAVE_LOOP_PATH) {
      ...
      std::string pose_graph_sequence_path = VINS_RESULT_PATH +
                                             "/pose_graph_path_" +
                                             to_string((it)->sequence) + ".csv";
      ofstream sequence_path_file(pose_graph_sequence_path, ios::app);
      sequence_path_file.setf(ios::fixed, ios::floatfield);
      sequence_path_file.precision(0);
      sequence_path_file << (it)->time_stamp * 1e9 << ",";
      sequence_path_file.precision(5);
      sequence_path_file << P.x() << "," << P.y() << "," << P.z() << ","
                         << Q.w() << "," << Q.x() << "," << Q.y() << ","
                         << Q.z() << "," << endl;
      sequence_path_file.close();
    }
...
void PoseGraph::savePoseGraph() {
  ...
  for (size_t k = 0; k < keyframe_vec.size(); k++) {
    ...
    fprintf(pFile,
            ...
            VIO_tmp_T.z(), VIO_tmp_Q.w(), VIO_tmp_Q.x(), VIO_tmp_Q.y(),
            VIO_tmp_Q.z(), tic.x(), tic.y(), tic.z(), ric.w(), ric.x(), ric.y(),
            ric.z(), (it)->loop_index, (it)->loop_info(0), (it)->loop_info(1),
    ...
    descriptor_path =
        POSE_GRAPH_SAVE_PATH + to_string((it)->index) + "_des.csv";
    feature_2d_path =
        POSE_GRAPH_SAVE_PATH + to_string((it)->index) + "_feature_2d.csv";
    ...
    for (size_t i = 0; i < (it)->feature_2d.size(); i++) {
      ...
      fprintf(feature_2d_file, "%f %f", (it)->feature_2d[i].x,
              (it)->feature_2d[i].y);
    }
...
void PoseGraph::loadPoseGraph() {
  ...
  while (fscanf(pFile,
                ...
                &index, &time_stamp, &VIO_Tx, &VIO_Ty, &VIO_Tz, &VIO_Qw,
                &VIO_Qx, &VIO_Qy, &VIO_Qz, &tic_x, &tic_y, &tic_z, &ric_w,
                &ric_x, &ric_y, &ric_z, &loop_index, &loop_info_0, &loop_info_1,
    ...
    string descriptor_path =
        POSE_GRAPH_SAVE_PATH + to_string(index) + "_des.csv";
    string feature_2d_path =
        POSE_GRAPH_SAVE_PATH + to_string(index) + "_feature_2d.csv";
    FILE *descriptor_file;
    FILE *feature_2d_file;
    ...
    KeyFrame *keyframe =
        new KeyFrame(0, time_stamp, VIO_T, VIO_R, tic, ric, point_3d,
                     feature_2d, point_des, feature_des);
    if (loop_index != -1) {
...
void PoseGraph::publishTF() {
  ...
  for (iter = sequence_align_world.begin(); iter != sequence_align_world.end();
    ...
    if (iter->second) {
      ...
      TF_t = sequence_r_drift_map[sequence] * sequence_w_t_s_map[sequence] +
             sequence_t_drift_map[sequence];
      ...
      br.sendTransform(tf::StampedTransform(transform, ros::Time::now(),
                                            "/global",
                                            "/drone_" + to_string(sequence)));
      // printf("%d tf t %f %f %f \n", sequence, TF_t.x(), TF_t.y(),TF_t.z());
      // printf("%d tf q %f %f %f %f \n", sequence, TF_q.w(),TF_q.x(), TF_q.y(),
      // TF_q.z());
    }
  }
}
